...
//  AOC2023
//
//  Created by Patrizia Heer on 03.12.2023.
//
...
final class AOC3 {
    ...
    func run() {
        ...
        for line in input.components(separatedBy: "\n") {
            let currLine = String(line)

            var i = 0

            while i < currLine.count {
                // Operation name[i] is not allowed in Swift, an alternative is
                let index = currLine.index(currLine.startIndex, offsetBy: i)
                let char = currLine[index]

                if char.isNumber {
                    //is number, check surronding for symbols
                    var hasSymbol = checkHasSurroundingSymbol(line: currLineNumber, char: i)

                    var currNumber = String(char)

                    //get following numbers
                    for z in i+1..<currLine.count {
                        let index = currLine.index(currLine.startIndex, offsetBy: z)
                        let char = currLine[index]

                        if char.isNumber {

                            currNumber = currNumber + String(char)
                            hasSymbol = hasSymbol || checkHasSurroundingSymbol(line: currLineNumber, char: z)
                        } else {
                            break
                        }
                    }

                    i = i + currNumber.count

                    if hasSymbol {
                        sum += Int(currNumber) ?? 0
                    }
                }
                i += 1
            }
    ...
    func partTwo() {
        ...
        for line in input.components(separatedBy: "\n") {
            ...
            while i < currLine.count {
                ...
                if char.isNumber {
                    ...
                    var indexes: Set = getStarIndex(line: currLineNumber, char: i)
                    //get following numbers
                    for z in i+1..<currLine.count {
                        let index = currLine.index(currLine.startIndex, offsetBy: z)
                        let char = currLine[index]
                        ...
                        if char.isNumber {
                            ...
                            getStarIndex(line: currLineNumber, char: z).forEach { indexes.insert($0)}
                    ...
                    indexes.forEach { index in
                        if var starNumbersList = starNumbers[index] {
                            starNumbersList.append(Int(currNumber) ?? 0)
                            starNumbers[index] = starNumbersList
                        } else {
                            starNumbers[index] = [Int(currNumber) ?? 0]
                        }
                    }
                }
                i += 1
        ...
        /*starNumbers.removeIf { elem in
            elem.value.count != 2
        }*/

        print(starNumbers)
        starNumbers.values.forEach {
            if $0.count == 2 {
                sum += ($0.first! * $0.last!)
            }
        }
    ...
    func getStarIndex(line: Int, char: Int) -> Set<IndexPath> {
        var result: Set<IndexPath> = []
        if line > 0 {
            let lineBefore = starMatrix[line-1]
            getLineStarIndex(line: lineBefore, char: char).forEach {
                result.insert(IndexPath(indexes: [line-1, $0]))
            }
        }
        if line < matrix.count - 1 {
            let lineAfter = starMatrix[line+1]
            getLineStarIndex(line: lineAfter, char: char).forEach {
                result.insert(IndexPath(indexes: [line+1, $0]))
            }
        }

        let currLine = starMatrix[line]
        getLineStarIndex(line: currLine, char: char).forEach {
            result.insert(IndexPath(indexes: [line, $0]))
        }
        return result
    }

    func getLineStarIndex(line: [Character], char: Int) -> [Int] {
        var result: [Int] = []

        if char > 0 && line[char-1] == "x" {
            result.append(char-1)
        } else if line[char] == "x" {
            result.append(char)
        } else if char < line.count - 1 && line[char+1] == "x" {
            result.append(char+1)
        }
    ...
    func checkHasSurroundingSymbol(line: Int, char: Int) -> Bool {
        var result = false
        if line > 0 {
            let lineBefore = matrix[line-1]
            result = checkLine(line: lineBefore, char: char)
        }
        if line < matrix.count - 1 {
            let lineAfter = matrix[line+1]
            result = result || checkLine(line: lineAfter, char: char)
        }

        let currLine = matrix[line]
        result = result || checkLine(line: currLine, char: char)
        return result

    }

    func checkLine(line: [Character], char: Int) -> Bool {

        if (char > 0 && line[char-1] == "x") || line[char] == "x" || (char < line.count - 1 && line[char+1] == "x") {
            return true
        }
        return false
    }
