...
//  AOC2023
//
//  Created by Patrizia Heer on 10.12.2023.
//
...
class AOC10 {
    ...
    func part1() {
        ...
        matrix = inputLines.components(separatedBy: .newlines).map { $0.map { String($0) }.compactMap { $0 } }
        matrixDistance = Array(repeating: Array(repeating: 0, count: matrix.first!.count), count: matrix.count)


        if let start = findStart() {
            let possibilites = checkSourrundings(x: start.0, y: start.1)

            for possibility in possibilites {
                matrixVisited = []
                visit(x: possibility.0, y: possibility.1)
            }
        }
        //visit(x: startX, y: startY)

        printMatrix(matrix: matrix)
        printMatrix(matrix: matrixDistance)
    ...
    func findHighest() -> Int {
        return matrixDistance.map { $0.max() ?? 0 }.max() ?? 0
    }

    func printMatrix(matrix: [[Any]]) {
        matrix.forEach { value in
            print(value)
        }
    }

    func checkSourrundings(x: Int, y: Int) -> [(Int, Int)] {
        //print("x: \(x) y: \(y)")
        var possibilities = [(Int, Int)]()

        if x > 0 && (matrix[x-1][y] == Directions.verticalPipe.rawValue || matrix[x-1][y] == Directions.turnSouthWest.rawValue || matrix[x-1][y] == Directions.turnSouthEast.rawValue) {
            possibilities.append((x-1, y))
        } 
        if x < matrix.count - 1 && (matrix[x+1][y] == Directions.verticalPipe.rawValue || matrix[x+1][y] == Directions.turnEastWest.rawValue || matrix[x+1][y] == Directions.turnNorthEast.rawValue) {
            possibilities.append((x+1, y))
        } 
        if y > 0 && (matrix[x][y-1] == Directions.horizontalPipe.rawValue || matrix[x][y-1] == Directions.turnEastWest.rawValue || matrix[x][y-1] == Directions.turnSouthEast.rawValue) {
            possibilities.append((x, y-1))
        } 
        if y < matrix[x].count - 1 && (matrix[x][y+1] == Directions.horizontalPipe.rawValue || matrix[x][y+1] == Directions.turnNorthEast.rawValue || matrix[x][y+1] == Directions.turnSouthWest.rawValue) {
            possibilities.append((x, y+1))
        }

        possibilities = possibilities.filter({ value in
            !matrixVisited.contains { $0 == value }
        })

        return possibilities
    }

    func visit(x: Int, y: Int, distance: Int = 0) {
        if distance + 1 < matrixDistance[x][y] || matrixDistance[x][y] == 0 {
            matrixDistance[x][y] = distance + 1
        }
        matrixVisited.append((x,y))

        let value = matrix[x][y]
        if value == Directions.start.rawValue {
            return
        }

        let possibilities = checkSourrundings(x: x, y: y)

        print(possibilities)

        for possibility in possibilities {

            visit(x: possibility.0, y: possibility.1, distance: distance + 1)

        }

    }

    var matrixDistance = [[Int]]()
    var matrixVisited = [(Int, Int)]()
    var matrix: [[String]] = []
    ...
    enum Directions: String {
        ...
        case horizontalPipe = "-"
        case turnEastWest = "L"
        case turnNorthEast = "J"
        case turnSouthWest = "7"
        case turnSouthEast = "F"
        case ground = "."
        case start = "S"
