...
//  AOC2023
//
//  Created by Patrizia Heer on 02.12.2023.
//
...
final class AOC2 {
    ...
    enum col: String {
        ...
        static func color(for text: String) -> col? {
            if text.contains(col.red.rawValue) {
                return .red
            } else if text.contains(col.blue.rawValue) {
                return .blue
            } else if text.contains(col.green.rawValue) {
                return .green
            }
            return nil
        }
    ...
    let loaded = [col.red: 12, col.green: 13, col.blue: 14]

    func run() {
        var sum = 0

        outerLoop: for input in inputLines.split(separator: "\n") {

            if let gameNumber = gameNumber(for: String(input)), let games = games(for: String(input)) {
                print("gameNumber: \(gameNumber)")

                for game in games.split(separator: ";") {
                    print(game)
                    var greenCount = 0
                    var redCount = 0
                    var blueCount = 0

                    for gameComponent in game.split(separator: ",") {

                        if let col = col.color(for: String(gameComponent)), let number = numberFromString(for: String(gameComponent)) {

                            switch col {
                                case .green: greenCount = greenCount + number
                                case .red: redCount = redCount + number
                                case .blue: blueCount = blueCount + number
                            }
                        }
                    }
                    print("green: \(greenCount), blue: \(blueCount), red: \(redCount)")



                    if greenCount > loaded[.green] ?? 0 || redCount > loaded[.red] ?? 0 || blueCount > loaded[.blue] ?? 0 {
                        continue outerLoop
                    }

                }
                print("sum before: \(sum) added \(gameNumber)")
                sum += gameNumber
            }
        }
                ...
                for game in games.split(separator: ";") {
                    ...
                    for gameComponent in game.split(separator: ",") {
                        ...
                        if let col = col.color(for: String(gameComponent)), let number = numberFromString(for: String(gameComponent)) {
                            ...
                            switch col {
                                case .green: gameGreen = gameGreen + number
                                case .red: gameRed = gameRed + number
                                case .blue: gameBlue = gameBlue + number
                            }
                        }
                    }

                    if gameGreen > highestGreen {
                        highestGreen = gameGreen
                    }

                    if gameRed > highestRed {
                        highestRed = gameRed
                    }

                    if gameBlue > highestBlue {
                        highestBlue = gameBlue
                    }
    ...
    func numberFromString(for text: String) -> Int? {
        let regex = /^[^\d]*(\d*)/
        if let stringNumber = matches(for: regex, in: text) {
            return Int(stringNumber)
        }
        return nil
    }

    func gameNumber(for text: String) -> Int? {

        if let index = text.firstIndex(of: ":") {
            let gameName = text[text.startIndex..<index]
            return numberFromString(for: String(gameName))
        }
    ...
    func games(for text: String) -> String? {
        print(text)
        if let index = text.firstIndex(of: ":") {
            let newIndex = text.index(after: index)
            let games = text[newIndex..<text.endIndex]
            return String(games)
        }
        return nil
    }
